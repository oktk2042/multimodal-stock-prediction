...
# 設定: いすゞ自動車 (Gate Score 0.41)
TARGET_CODE = 7202
TARGET_DATE_STR = "2025-07-01"  # Gateが最大になった日
MODE = "High Attention"
EVENT_LABEL = "Strong News Impact\n(Gate Score: 0.41)"
NEWS_SENTIMENT = "Positive / High Impact"

# 共通設定
SHIFT_DAYS = 7      # 予測線の左シフト量
WINDOW_DAYS = 60    # 表示期間（前後60日）
...
def plot_refined_case(target_code, target_date_str, mode, event_label, news_sentiment, save_name):
    ...
    # 上段: 株価チャート
    # -----------------------------
    ax1.plot(plot_data['Target_Date'], plot_data['Actual'], color='#333333', label='Actual Price', linewidth=2.0, alpha=0.8)
    ax1.plot(plot_data['Pred_Plot_Date'], plot_data['Pred'], color='#d62728', linestyle='--', label='Prediction (AI)', linewidth=2.5)
    ...
    y_range = y_max - y_min
    ax1.set_ylim(y_min - y_range * 0.1, y_max + y_range * 0.4) 
    
    # X軸の範囲（データがある範囲でしっかり表示）
    ...
    ax1.annotate(
        ...
        xytext=(0.5, 0.95), textcoords='axes fraction', # 中央上部固定
        ha='center', va='top',
...
plot_refined_case(
    ...
    save_name="Refined_Case1_Isuzu_HighGate.png"
)