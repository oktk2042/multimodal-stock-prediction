...
TARGET_CODE = 6146  # ディスコ
TARGET_DATE = "2025-01-20"  # Gateが閉じていたのに株価が急変した日
EVENT_LABEL = "High Volatility\n(Regarded as Noise)"  # ラベル
...
def plot_closed_case():
    ...
    ax1.set_title("Case Study: DISCO (6146) - Noise Blocking Effect", fontsize=14)
    ...
    # Gate Closed強調
    ax2.scatter([center_date], [gate_val], color="gray", s=50, zorder=5)
    ax2.annotate(
        f"Gate CLOSED\n(Score: {gate_val:.2f})",
        xy=(center_date, gate_val),
        xytext=(center_date + timedelta(days=2), gate_val + 0.1),
        color="gray",
        fontweight="bold",
    ...
    plt.tight_layout()
    save_name = "Case2_Disco_Closed_Final.png"
